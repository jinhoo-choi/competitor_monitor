...
HARD_EXCLUDE_PATTERNS = [
    ...
    # 애널리스트 목표주가·투자의견·실적전망 리포트 — 경쟁사 "서비스/사업" 동향이 아닌
    # 증권사가 타사 "주가"를 전망하는 루틴 기사 (7/11 오탐 사례: "2분기 호실적 기대…배당 확대는 제한적")
    r"목표주가", r"목표가\s*(상향|하향|유지|제시|올려)", r"투자의견",
    r"컨센서스", r"매수\s*의견", r"[0-9]분기\s*(호실적|실적)\s*(기대|전망)",
    r"실적\s*전망.{0,10}(상향|하향|유지)",
...
- 경쟁사 서비스·사업이 후퇴하거나 차질이 생기는 기사는 제외
  ...
  ⚠️ 단, "제재 감경", "중징계 완화", "인가 재개", "길 터준다", "인가 획득"처럼
  규제·제재 이슈가 경쟁사에 유리하게 풀려 신사업(발행어음 등) 진출이 가능해지는
  기사는 정반대이므로 절대 여기 해당 안 됨 — 아래 [반드시 포함] 참조.
  핵심 판단 기준은 단어가 아니라 "최종 결과가 경쟁사에 유리한지 불리한지"임.
...
[반드시 포함 — 인가 획득·제재 완화로 신사업이 가능해지는 기사]
경쟁사가 규제 장벽을 넘어 신사업(발행어음, 종합금융투자사업자 지정 등) 진출이
가능해지는 기사는 무조건 relevant:true.
예) "중징계 감경", "제재 완화", "인가 재개", "길 터준다", "인가 획득", "조건부 인가"
신규 사업권 획득은 경쟁사 사업영역 확대로 직결되므로 위 [제외] 규칙의
"인가 제동/거부"와 정반대 방향임 — 혼동 금지.
...
def _sanitize_header_text(text: str) -> str:
    ...
    거부(7/18 반송 실사례, 원인=전각대시·이모지). ASCII는 거부 사유가 될 수 없으므로
    ASCII 전체(0x20~0x7E)를 통째로 허용해 'M&A', '!', '?' 등이 깨지지 않게 한다.
    한글(완성형+자모)도 허용, 그 외(이모지·전각기호 등)만 제거한다."""
